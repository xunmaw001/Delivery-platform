...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussshanghuxinxi');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discussshanghuxinxi');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discussshanghuxinxi', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscussshanghuxinxi', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscussshanghuxinxi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discussshanghuxinxi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discussshanghuxinxi', this.detail);
				this.$utils.msg('审核成功');
